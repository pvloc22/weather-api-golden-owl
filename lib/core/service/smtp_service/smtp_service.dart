...
import 'dart:convert';
import 'package:golden_owl/core/service/firebase_email_service.dart';
...
class SmtpService {
  ...
  // Firebase Email Service for web
  final _firebaseEmailService = FirebaseEmailService();
  ...
  Future<bool> sendVerificationEmail({required String recipientEmail, required String otp}) async {
    // Use Firebase Cloud Functions for web
    if (kIsWeb) {
      return await _firebaseEmailService.sendVerificationEmail(recipientEmail: recipientEmail, otp: otp);
    }
  ...
  Future<bool> sendSubscriptionConfirmation({
    ...
    // Use Firebase Cloud Functions for web
    if (kIsWeb) {
      return await _firebaseEmailService.sendSubscriptionConfirmation(
    ...
    try {
      ...
      final cityName = weatherData?['city_name'] ?? location;
  ...
  Future<bool> sendDailyForecast({
    ...
    // Use Firebase Cloud Functions for web
    if (kIsWeb) {
      return await _firebaseEmailService.sendDailyForecast(
        recipientEmail: recipientEmail,
        location: location,
    ...
    try {
      ...
      final cityName = forecastData['city_name'] ?? location;
  ...
  Future<bool> sendUnsubscribeConfirmation({required String recipientEmail}) async {
    // Use Firebase Cloud Functions for web
    if (kIsWeb) {
      return await _firebaseEmailService.sendUnsubscribeConfirmation(recipientEmail: recipientEmail);
    }
  ...
  Future<Map<String, bool>> bulkSendForecast({
    required List<Map<String, String>> recipients, // List of {email, location}
    ...
    for (var recipient in recipients) {
      ...
      final location = recipient['location']!;

      if (!locationGroups.containsKey(location)) {
        locationGroups[location] = [];
      }
      locationGroups[location]!.add(email);
    }
